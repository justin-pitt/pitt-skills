...
| `references/story-audit.md` | Auditing an **existing** storyboard for AI-agent optimization: topology (agent chains vs central routing), tool counts per agent, sequential-vs-parallel logic, AI-necessity substitution test. Maps each check to `tines-mcp` read tools (`tines_story_overview`, `tines_get_action`, `tines_search_story_contents`); emits an Optimized / Refactor-recommended verdict. Cites the design-time refs rather than restating them |
