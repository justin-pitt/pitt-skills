---
applyTo: "**"
description: Use whenever the user mentions Tufin, TOS, SecureTrack, SecureChange, SecureApp, USP, TQL, Tufin REST or GraphQL API, Designer, Verifier, access requests, policy violations, zone matrices, network zones, firewall rule cleanup, rule recertification, decommission tickets, topology map, path analysis, OPM agents, TufinMate, Workflow Integrator, or any policy-management or change-automation task involving Tufin. Also trigger when the user wants to write Tufin REST/GraphQL calls, build or modify SecureChange workflows, write TQL queries, design SOAR playbooks that pull policy or topology data from SecureTrack, automate access-request submission, integrate Tufin with ITSM (ServiceNow), SOAR (XSOAR, Tines, Torq), or SIEM, or integrate Tufin into M&A network onboarding. Reference targets TOS Aurora R25-1 (example deployment: 25.1 PHF3).
---

# Tufin Orchestration Suite (TOS) Aurora

Reference skill for Tufin TOS R25-1 (example deployment: 25.1 PHF3). Covers the three platform components, the REST and GraphQL APIs, TQL, the Unified Security Policy, change automation, the Workflow Integrator extension, and integration patterns relevant to a security automation engineering team.
...
2. **Co-administering the on-prem TOS instance with Network Security.** The network security team owns the platform; an automation engineer may help admin it. Admin tasks in scope: API service accounts, RBAC, WFI configuration, custom workflow scripts, USP exception management, ticket mapping, audit log forwarding, integration with ServiceNow and SOAR. Out of scope for this skill: bare-metal install, version upgrades, backup/restore procedures, hardware (G4/G4.5 appliance) maintenance. Those live in the Tufin KC.

A community Tufin MCP server exists at `github.com/stonecircle82/tufin-mcp` (Python, FastAPI, RBAC, basic SecureChange + SecureTrack endpoint coverage). Worth reading before starting from scratch; may not match your needs but useful as a reference implementation.
...
## R25-1 Version Notes (example deployment: 25.1 PHF3)
...
- **TufinMate** (AI assistant) GA. Three flavors: TufinMate for IT (Teams), for SOC (Microsoft Copilot for Security), for network security engineers (later in 2025).
...
If you're working in an environment running PHF1+, USP zone-based exceptions and dynamic polling are both available; environments on PHF3 get them by default. Confirm AWS NAT Gateway internet-path coverage if your onboarding work hits AWS-egress access requests.
