...
**Filter quirk:** `playbooks/get` returns HTTP 400 on roughly 15-20% of `[ORG]`-prefixed names (no obvious pattern - `[ORG] Successful guest user invitation` works, `[ORG] XCloud Cryptomining` 400s). Same payload shape that succeeds for non-bracketed names. Workaround: try insert as-is and parse any colliding internal UUID out of the error response, then retry with the YAML's `id:` swapped to that UUID.
...
|----------|---------|--------------|-------|
| `POST /xsoar/inv-playbook/new` | Attach a playbook to one or more existing alerts (bulk) | `{"playbookId": "<id>", "alertIds": ["<id1>","<id2>"], "version": -1}` | **Max 10 alert IDs per call.** Replaces any playbook already running on those alerts. `playbookId` is the YAML `id:` field (UUID for system pack content, slug like `<org>-...` for custom-authored), NOT the display name. |
