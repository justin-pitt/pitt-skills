...
Sub-playbook tasks (`type: playbook`) need careful error-handling configuration when called from a parent — a hard internal failure with no output context can halt the parent even with `continueonerror: true` set. The reliable fail-open shape mirrors what task 2 (Threat Intel) and the standard fail-open preamble use:
...
This matters for any default that references `${issue}` or `${alert}` — the sub's input default chain runs in the SUB's empty context, not the parent's, so the default resolves to empty. A common Dedup utility's default identifier (`substringFrom("at", issue.rawName) | trim`) is doubly broken: it relies on parent context AND the substringFrom("at") chain produces nonsense identifiers for alerts containing "at" in their static text (e.g., "Attachment"). Always override with an explicit identifier on the wrapper task.
