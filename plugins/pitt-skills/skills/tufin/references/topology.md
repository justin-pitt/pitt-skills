...
A common device mix is **Palo Alto NGFW** (full coverage, native) alongside a non-native vendor such as **Akamai** (not natively supported by Tufin; would need an OPM agent or excluded from TOS coverage). When a non-native vendor is introduced (for example during network onboarding), the path is an OPM agent (Pro Services-developed) or onboarding the device's traffic via syslog only.
