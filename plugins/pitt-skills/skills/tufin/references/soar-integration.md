...
How to wire Tufin into security automation. Covers the XSOAR Tufin pack, the Tines/Torq webhook patterns, ServiceNow Workflow Integrator, and common security-automation playbooks.
...
The XSOAR pack maps cleanly to Tines/Torq HTTP Request actions. For Tines specifically:
...
If ServiceNow IRM is owned by a separate team, the Workflow Integrator App is the lower-friction path. Validate it carries through the IRM custom fields you need before committing.
...
Tufin's **Vulnerability-based Change Automation App (VCA)** does this end to end if you license it. If you run Rapid7/Tenable/Qualys, VCA is the simpler path. Without it, the playbook above replicates the value with more code.
