...
WFI is the Tufin Extension that wires SecureChange workflows to ITSM and other REST-capable systems without writing custom mediator scripts. Free with SecureChange+. The intended path for ServiceNow integration at most shops.
...
WFI is configured by the platform admin (typically the network security team). When admin'ing WFI, treat the JSON templates as code: source-control them, review changes, and test in a non-prod workflow before enabling on production tickets.
